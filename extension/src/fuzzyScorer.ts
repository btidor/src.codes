...
export function scoreFuzzy(target: string, query: string, queryLower: string, allowNonContiguousMatches: boolean): number {
    if (!target || !query) {
...
function doScoreFuzzy(query: string, queryLower: string, queryLength: number, target: string, targetLower: string, targetLength: number, allowNonContiguousMatches: boolean): number {
    const scores: number[] = [];
...
function computeCharScore(queryCharAtIndex: string, queryLowerCharAtIndex: string, target: string, targetLower: string, targetIndex: number, matchesSequenceLength: number): number {
    ...
    if (targetIndex === 0) {
        ...
        else if (target.charCodeAt(targetIndex) >= 65 && target.charCodeAt(targetIndex) <= 90) {
            score += 2;
