...
type Symbols = Map<string, vscode.SymbolInformation[]>;
...
export default class SymbolsClient {
    ...
    private locals: Map<string, Thenable<Symbols>>;
            ...
            .then(res => {
                ...
                for (const line of res.data.split("\n")) {
                    if (line.startsWith("### ")) {
                        ...
                        if (info) {
                            if (!syms.has(info.name)) {
                                syms.set(info.name, []);
                            }
                            syms.get(info.name).push(info);
                        }
    ...
    listPackageSymbols(pkg: Package): Thenable<Symbols> {
        ...
        return this.locals.get(pkg.name)!;
    }
